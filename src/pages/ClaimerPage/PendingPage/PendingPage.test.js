...
import PendingClaims from './PendingPage';
...
describe('PendingClaims Component', () => {
    ...
    // Test 2: Function - Filter claims with "Pending" status
    test('Filters claims with "Pending" status correctly', () => {
        render(
            <Provider store={store}>
                <PendingClaims />
            </Provider>
        ...
        const expectedPendingClaims = mockClaims.filter(claim => claim.status === 'Pending');
        expect(screen.getByTestId('claims-count').textContent).toBe(expectedPendingClaims.length.toString());
    });
    ...
    test('Renders ClaimsTable with correct title', () => {
        ...
        expect(screen.getByText('Summary of Pending Claims')).toBeInTheDocument();
    });
