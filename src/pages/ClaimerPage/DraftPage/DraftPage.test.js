...
import DraftClaims from './DraftPage';
...
describe('DraftClaims Component', () => {
    let store;
    const mockClaims = [
    ...
    // Test 2: Function - Filter claims with "Draft" status
    test('Filters claims with "Draft" status correctly', () => {
        render(
            <Provider store={store}>
                <DraftClaims />
            </Provider>
        ...
        const expectedDraftClaims = mockClaims.filter(claim => claim.status === 'Draft');
        expect(screen.getByTestId('claims-count').textContent).toBe(expectedDraftClaims.length.toString());
    });
    ...
    test('Renders page title correctly', () => {
        ...
        expect(screen.getByText('Pages > Draft')).toBeInTheDocument();
    });
    ...
    test('Renders ClaimsTable with correct title', () => {
        ...
        expect(screen.getByText('Summary of Draft Claims')).toBeInTheDocument();
    });
