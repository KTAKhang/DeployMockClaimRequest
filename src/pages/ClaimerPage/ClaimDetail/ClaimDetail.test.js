...
jest.mock("../../../redux/actions/claimerActions", () => ({
  fetchClaimDetailRequest: jest.fn(() => ({
...
// Mock components
jest.mock("../../../components/Popup/Claim/UpdateClaimForm", () => {
  ...
  test("Renders claim details correctly", () => {
    ...
    expect(
      screen.getByText("Pending", {
        selector: "p.font-semibold.text-yellow-500",
      })
    ).toBeInTheDocument();
  });
  ...
  test("Displays claim status correctly", () => {
    ...
    waitFor(() => {
      ...
      expect(statusContainer).toHaveClass(
        /text-(yellow|green|red|blue|pink|gray)-500/
      );
    });
  });
