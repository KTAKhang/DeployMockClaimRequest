...
import { LABELS, truncateString } from "./strings";
import {
  filterClaims,
  sortClaims,
...
export default function ClaimsTable({
  ...
  switch (filterCondition) {
    case FILTER_CONDITIONS.FOR_MY_VETTING:
      mode = MODES.VETTING;
      break;
    case FILTER_CONDITIONS.CLAIMER_DASHBOARD:
      mode = MODES.CLAIMER_DASHBOARD;
      break;
    case FILTER_CONDITIONS.CLAIM_MANAGEMENT:
      mode = MODES.CLAIM_MANAGEMENT;
      break;
    default:
      mode = MODES.HISTORY;
  }
  ...
  const handleViewDetail = (id) => {
    if (onViewDetail) {
      console.log("Using custom view handler from props");
      onViewDetail(id);
      return;
    }
      ...
      <div className="w-full max-w-[350px] sm:max-w-[550px] md:max-w-none bg-white shadow-lg border rounded-xl p-3 sm:p-4 md:p-6 overflow-hidden">
        ...
        <div className={`w-full ${isMobile ? "px-1" : "px-2 sm:px-3 md:px-4"}`}>
          <div
            ...
            style={{
              minHeight: paginatedClaims.length > 0 ? "440px" : "auto",
            }}
          >
